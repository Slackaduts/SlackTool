...
async def noclip_up(foreground_client : wizwalker.Client, speed_constant : int = 580, speed_adjusted : bool = True, down : bool = False, debug : bool = False):
	...
	logger.info('Noclip is currently disabled as it does not work properly anymore.')
	# if down:
	# 	speed_constant *= -1
	# 	if debug:
...
async def noclip_forward(foreground_client : wizwalker.Client, speed_constant : int = 580, speed_adjusted : bool = True, backward : bool = False, debug : bool = False):
	...
	# 	await foreground_client.teleport(frontal_xyz, move_after=False)
	# else:
	# 	logger.error('Foreground client does not exist, skipping teleport.')
...
async def main():
	...
	# 			await asyncio.sleep(1)
	# 			if client in walker.clients:
	# 				logger.debug(f'Client {client.title} in combat, handling questing.')
	...
	async def anti_afk_loop():
		...
		async def async_anti_afk(client: Client):
			while True:
				...
				if SlackTeleport(client).calc_Distance(client_xyz, client_xyz_2) < 5 and not await client.in_battle():
	...
	try:
		...
		while True:
			await asyncio.wait([foreground_client_switching_task, speed_switching_task, combat_loop_task, assign_foreground_clients_task, dialogue_loop_task, anti_afk_loop_task])
