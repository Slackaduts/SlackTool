...
pvp_strategy = ['AOE Global', 'AOE Defense Aura', 'AOE Offensive Shadow Creature', 'AOE DOT', 'AOE Hit', 'Enemy Select DOT', 'Enemy Select Hit', 'Enemy Select Hit Divide']
...
class SlackFighter(CombatHandler):
	...
	async def is_fighting(self):
		'''New accurate check for if we're in combat, using combat member reading'''
	...
	@logger.catch()
	async def discard_useless(self):
		...
		while True:
			if self.selected_spell:
				...
				if 'AOE' in selected_spell_logic:
					...
					await self.selected_spell.cast(None, sleep_time=0.18)
					...
					await self.selected_spell.cast(self.selected_enemy, sleep_time=0.18)
					...
					await self.selected_spell.cast(self.selected_ally, sleep_time=0.18)
					...
					match effect_types[effect]:
						...
						case SpellEffects.modify_incoming_damage:
							...
							if ward_param < 0:
								...
								caster_pierce += await effect_params[effect]
								if ward_param > 0:
									ward_param = 0
								if caster_pierce < 0:
									caster_pierce = 0
